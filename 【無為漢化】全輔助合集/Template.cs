...
namespace Support
{
    public class Template : PluginBase
    {
        ...
        public override void ComboMenu(Menu config)
        {
            ...
            config.AddSlider("ComboCountR", "幾個敵人使用大招", 2, 0, 5);
            config.AddSlider("ComboHealthR", "剩餘血量開啟大招ㄧ", 20, 1, 100);
        }
        ...
        {
            config.AddBool("GapcloserQ", "使用 Q 防突進ㄧ", true);
            config.AddBool("GapcloserW", "使用 W 防突進ㄧ", true);
            config.AddBool("GapcloserE", "使用 E 防突進ㄧ", true);
            config.AddBool("GapcloserR", "使用 R 防突進ㄧ", true);

            config.AddBool("InterruptQ", "使用 Q 打斷法術", true);
            config.AddBool("InterruptW", "使用 W 打斷法術", true);
            config.AddBool("InterruptE", "使用 E 打斷法術", true);
            config.AddBool("InterruptR", "使用 R 打斷法術", true);
        }
    }
}
