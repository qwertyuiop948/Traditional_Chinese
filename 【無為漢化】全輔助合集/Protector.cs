...
namespace Support
{
    ...
    internal class Protector
    {
        ...
        {
			Game.PrintChat("<font color=\"#00BFFF\">====== 成功載入:【無為漢化】全輔助合集 ======</font>");
			Game.PrintChat("<font color=\"#00BFFF\">====== 包含英雄====== </font>");
			Game.PrintChat("<font color=\"#00BFFF\">ㄧ牛頭ㄧ機器人ㄧ布朗姆ㄧ費德提克ㄧ風女ㄧ卡瑪ㄧ蕾歐娜ㄧ露露ㄧ莫甘娜ㄧ娜米ㄧ索娜ㄧ塔里克ㄧ瑟雷西ㄧ極靈ㄧ枷蘿ㄧ</font>");
			Game.PrintChat("<font color=\"#00BFFF\">更多漢化腳本請加  L#漢化群：386289593</font>");
            Menu = new Menu("Support: 支持工具", "Protector", true);

            // detector
            var detector = Menu.AddSubMenu(new Menu("探測器ㄧ", "Detector"));

            // detector targeted
            var targeted = detector.AddSubMenu(new Menu("針對", "Targeted"));
            targeted.AddItem(new MenuItem("TargetedActive", "啟用").SetValue(true));
            ...
            // detector skillshots
            var skillshot = detector.AddSubMenu(new Menu("技能標示ㄧ", "Skillshots"));
            skillshot.AddItem(new MenuItem("SkillshotsActive", "啟用").SetValue(true));
            skillshot.AddItem(new MenuItem("IsAboutToHitTime", "時間延遲").SetValue(new Slider(200, 0, 400)));
            foreach (var ally in ObjectManager.Get<Obj_AI_Hero>().Where(h => h.IsAlly))
            {
            ...
            // spells
            var spells = Menu.AddSubMenu(new Menu("法術", "Spells"));
            ...
            // items
            var items = Menu.AddSubMenu(new Menu("項目", "Items"));
            ...
            // misc
            var misc = Menu.AddSubMenu(new Menu("雜項", "Misc"));
