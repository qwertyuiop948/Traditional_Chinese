...
namespace Support.Disabled
{
    public class Soraka : PluginBase
    {
        ...
        public override void ComboMenu(Menu config)
        {
            ...
            config.AddSlider("ComboCountR", "目標範圍內的結果", 2, 0, 5);
            config.AddSlider("ComboHealthR", "血量低於%使用大招", 20, 1, 100);
        }
        ...
        {
            config.AddBool("GapcloserQ", "使用 Q 打斷突進", true);
            config.AddBool("GapcloserW", "使用 W 打斷突進", true);
            config.AddBool("GapcloserE", "使用 E 打斷突進", true);
            config.AddBool("GapcloserR", "使用 R 打斷突進", true);

            config.AddBool("InterruptQ", "使用 Q 打斷技能", true);
            config.AddBool("InterruptW", "使用 W 打斷技能", true);
            config.AddBool("InterruptE", "使用 E 打斷技能", true);
            config.AddBool("InterruptR", "使用 R 打斷技能", true);
        }
    }
}