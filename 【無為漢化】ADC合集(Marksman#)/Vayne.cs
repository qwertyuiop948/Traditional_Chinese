...
namespace Marksman
{
    internal class Vayne : Champion
    {
        ...
        {
            config.AddItem(
                new MenuItem("UseET" + Id, "使用E (鎖定)").SetValue(
                    new KeyBind("T".ToCharArray()[0], KeyBindType.Toggle)));
            config.AddItem(new MenuItem("UseEInterrupt" + Id, "使用E (鎖定)").SetValue(true));
            config.AddItem(
                new MenuItem("PushDistance" + Id, "E距離").SetValue(new Slider(425, 475, 300)));
            return true;
        }
