...
namespace Marksman
{
    internal class Urgot : Champion
    {
        ...
        public override bool ComboMenu(Menu config)
        {
            ...
            config.AddSubMenu(new Menu("大招選擇 1", "UltOpt1"));

            config.SubMenu("UltOpt1")
                .AddItem(
                    new MenuItem("UltOp1" + Id, "傳送隊友炮塔").SetValue(new KeyBind("T".ToCharArray()[0],
                        KeyBindType.Press)));

            config.AddSubMenu(new Menu("大招選擇 2", "UltOpt2"));
            config.SubMenu("UltOpt2")
                .AddItem(
                    new MenuItem("UltOp2" + Id, "我的團隊傳送").SetValue(new KeyBind("G".ToCharArray()[0],
                        KeyBindType.Press)));
            config.SubMenu("UltOpt2")
                .AddItem(new MenuItem("UltOp2Count" + Id, "最小的對人數量").SetValue(new Slider(1, 1, 5)));
