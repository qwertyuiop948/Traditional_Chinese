...
namespace Marksman
{
    ...
    internal class Draven : Champion
    {
        ...
        public override bool DrawingMenu(Menu config)
        {
            ...
            config.AddItem(
                new MenuItem("DrawR", "R 範圍(2000 單位)").SetValue(
                    new Circle(true, Color.FromArgb(100, 255, 0, 255))));
            config.AddItem(
                new MenuItem("DrawOrbwalk", "顯示走砍位置").SetValue(new Circle(true, Color.Yellow)));
            config.AddItem(new MenuItem("DrawReticles", "顯示畫線").SetValue(new Circle(true, Color.Green)));
            return true;
        }

        public override bool MiscMenu(Menu config)
        {
            config.AddItem(new MenuItem("maxqamount", "最多同時使用多少Q").SetValue(new Slider(2, 4, 1)));
            config.AddItem(new MenuItem("EGapCloser", "自動E防突進").SetValue(true));
            config.AddItem(new MenuItem("EInterruptable", "自動E打斷技能").SetValue(true));
            config.AddItem(new MenuItem("RManualCast", "手動大招(2000 範圍)"))
                .SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Press));
            config.AddItem(new MenuItem("Epeel", "剝離自我與 E").SetValue(true));
            config.AddItem(new MenuItem("CatchRadius", "捕捉斧頭半徑").SetValue(new Slider(600, 200, 1000)));
            return true;
        }
