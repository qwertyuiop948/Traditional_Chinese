...
namespace Marksman
{
    internal class Caitlyn : Champion // Base done by xQx, Drawings and improvements added by Dibes.
    {
        ...
        {
            config.AddItem(
                new MenuItem("UltHelp" + Id, "大招的目標 R").SetValue(new KeyBind("R".ToCharArray()[0],
                    KeyBindType.Press)));
            config.AddItem(
                new MenuItem("UseEQC" + Id, "使用 E-Q 連招").SetValue(new KeyBind("T".ToCharArray()[0],
                    KeyBindType.Press)));
            config.AddItem(
                new MenuItem("Dash" + Id, "跟隨鼠標").SetValue(new KeyBind("Z".ToCharArray()[0], KeyBindType.Press)));
            return true;
        }
