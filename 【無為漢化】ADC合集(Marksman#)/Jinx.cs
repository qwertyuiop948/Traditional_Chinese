...
namespace Marksman
{
    internal class Jinx : Champion
    {
        ...
        {
            config.AddItem(new MenuItem("SwapQ" + Id, "總是切換輕槍").SetValue(false));
            return true;
        }

        public override bool MiscMenu(Menu config)
        {
            config.AddItem(new MenuItem("SwapDistance" + Id, "距離不夠切換Q").SetValue(true));
            config.AddItem(new MenuItem("SwapAOE" + Id, "切換QAOE").SetValue(false));
            config.AddItem(new MenuItem("MinWRange" + Id, "MinW範圍").SetValue(new Slider(525 + 65 * 2, 0, 1200)));
            config.AddItem(new MenuItem("AutoEI" + Id, "自動E靜止").SetValue(true));
            config.AddItem(new MenuItem("AutoES" + Id, "自動E受控").SetValue(true));
            config.AddItem(new MenuItem("AutoED" + Id, "自動E突進").SetValue(false));
            config.AddItem(
                new MenuItem("CastR" + Id, "半自動R(2000)").SetValue(
                    new KeyBind("T".ToCharArray()[0], KeyBindType.Press)));
            config.AddItem(new MenuItem("ROverKill" + Id, "檢查R可殺").SetValue(true));
            config.AddItem(new MenuItem("MinRRange" + Id, "Min R範圍").SetValue(new Slider(300, 0, 1500)));
            config.AddItem(new MenuItem("MaxRRange" + Id, "Max R範圍").SetValue(new Slider(1700, 0, 4000)));
            return true;
        }
        ...
        {
            config.AddItem(
                new MenuItem("DrawQBound" + Id, "Q範圍").SetValue(
                    new Circle(true, Color.FromArgb(100, 255, 0, 0))));
            config.AddItem(
                new MenuItem("DrawW" + Id, "W範圍").SetValue(new Circle(false, Color.CornflowerBlue)));
            return true;
        }
