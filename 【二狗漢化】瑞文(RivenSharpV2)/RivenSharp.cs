    ...
    class RivenSharp
    {
        ...
        private static void onLoad(EventArgs args)
        {
           ...
           Config.AddSubMenu(TargetSelectorMenu);
            ...
            Config.SubMenu("combo").AddItem(new MenuItem("forceQE", " 先|E 後|Q")).SetValue(true);
            Config.SubMenu("combo").AddItem(new MenuItem("packets", "使用 封包")).SetValue(true);

            //Haras
            Config.AddSubMenu(new Menu("騷擾 Sharp", "haras"));
            Config.SubMenu("haras").AddItem(new MenuItem("doHarasE", "使用 E 騷擾")).SetValue(new KeyBind('G', KeyBindType.Press, false));
            Config.SubMenu("haras").AddItem(new MenuItem("doHarasQ", "使用 Q 騷擾")).SetValue(new KeyBind('T', KeyBindType.Press, false));

            //Drawing
            Config.AddSubMenu(new Menu("範圍 Sharp", "draw"));
            Config.SubMenu("draw").AddItem(new MenuItem("doDraw", "禁用 範圍")).SetValue(false);
            Config.SubMenu("draw").AddItem(new MenuItem("drawHp", "顯示 傷害 血量")).SetValue(true);

            //Debug
            Config.AddSubMenu(new Menu("調試 Sharp", "debug"));
            Config.SubMenu("debug").AddItem(new MenuItem("db_targ", "調試 目標")).SetValue(new KeyBind('0', KeyBindType.Press, false));
