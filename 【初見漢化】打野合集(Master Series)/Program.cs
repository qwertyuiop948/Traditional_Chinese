...
namespace Master
{
    class Program
    {
        ...
        private static void OnGameLoad(EventArgs args)
        {
            ...
            Config.AddSubMenu(new Menu("目標選擇", "TSSettings"));
            Config.SubMenu("TSSettings").AddItem(new MenuItem("tsMode", "模式").SetValue(new StringList(new[] { "|自動|", "|最多AD|", "|最多AP|", "|最少攻擊|", "|最少平A|", "|最低HP|", "|最近|", "|鼠標附近|" })));
            Config.SubMenu("TSSettings").AddItem(new MenuItem("tsFocus", "強制目標").SetValue(true));
            Config.SubMenu("TSSettings").AddItem(new MenuItem("tsDraw", "顯示目標").SetValue(true));
