...
namespace Master
{
    class Riven : Program
    {
        ...
        public Riven()
        {
            ...
            Config.SubMenu("hsettings").AddItem(new MenuItem(Name + "harMode", "如果以上HP使用騷擾").SetValue(new Slider(20, 1)));
            ...
            Config.SubMenu("miscs").AddItem(new MenuItem(Name + "cancelAni", "|動畫取消模式|").SetValue(new StringList(new[] { "Dance", "Laugh", "Move" }, 2)));
            Config.SubMenu("miscs").AddItem(new MenuItem(Name + "useDodgeE", "|使用e閃躲技能|").SetValue(true));
            Config.SubMenu("miscs").AddItem(new MenuItem(Name + "useAntiW", "|使用W反差距接近|").SetValue(true));
            Config.SubMenu("miscs").AddItem(new MenuItem(Name + "useInterW", "|使用W打斷|").SetValue(true));
