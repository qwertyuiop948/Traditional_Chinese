        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            spawn =
                ObjectManager.Get<GameObject>()
                    ...
                    .Position;
            menu = new Menu("索拉卡 掛機", "slack", true);
            menu.AddItem(new MenuItem("on", "開始 掛機!").SetValue(new KeyBind(32, KeyBindType.Toggle)));
            menu.AddItem(new MenuItem("user", "使用 R").SetValue(true));
            menu.AddItem(new MenuItem("usew", "使用 W").SetValue(true));
            menu.AddItem(new MenuItem("allyhpw", "使用W|隊友血量").SetValue(new Slider(30, 0, 93)));
            menu.AddItem(new MenuItem("wabovehp", "使用W|自己血量").SetValue(new Slider(20, 0, 99)));
            menu.AddItem(new MenuItem("allyhpr", "使用R|隊友血量").SetValue(new Slider(30, 0, 50)));
            menu.AddItem(new MenuItem("hpb", "使用回程|自己血量").SetValue(new Slider(15, 0, 80)));
            menu.AddItem(new MenuItem("talk", "自動聊天").SetValue(true));
            //Game.PrintChat("in2");
            menu.AddSubMenu(new Menu("隊友跟隨:", "follower"));
