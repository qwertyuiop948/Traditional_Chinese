...
//                    .SetValue(ObjectManager.Player.Spellbook.GetSpell(SpellSlot.R).Name == "LucianR");
        ...
        public override bool HarassMenu(Menu config)
        {
            ...
            config.AddItem(
                new MenuItem("UseQTH" + Id, "使用Q (切換)").SetValue(new KeyBind("T".ToCharArray()[0],
                    KeyBindType.Toggle)));
            config.AddItem(new MenuItem("Cx",""));
            config.AddItem(new MenuItem("UseQExtendedH" + Id, "使用擴展EQ").SetValue(true));
            config.AddItem(
                new MenuItem("UseQExtendedTH" + Id, "使用EQ (切換").SetValue(new KeyBind("H".ToCharArray()[0],
                    KeyBindType.Toggle)));
            config.AddItem(new MenuItem("Cx", ""));
        ...
        {
            config.AddItem(new MenuItem("Passive" + Id, "檢查被動").SetValue(true));
            return true;
        }
        ...
        public override bool DrawingMenu(Menu config)
        {
            ...
            config.AddItem(new MenuItem("DrawQ2" + Id, "EQ 範圍").SetValue(new Circle(true, Color.Gray)));
