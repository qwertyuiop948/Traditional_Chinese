...
namespace Marksman
{
    internal class Varus : Champion
    {
        ...
        public override bool ComboMenu(Menu config)
        {
            config.AddItem(
                new MenuItem("UseQC" + Id, "Q").SetValue(
                    new StringList(new[] {"Off", "每次", "W 層數", "最大W層數"}, 3)));
            config.AddItem(
                new MenuItem("UseQMinChargeC" + Id, "最小 Q 距離").SetValue(new Slider(1000, 250, 1400)));
        ...
        public override bool HarassMenu(Menu config)
        {
            ...
            config.AddItem(
                new MenuItem("UseETH" + Id, "使用 E (切換)").SetValue(new KeyBind("H".ToCharArray()[0],
                    KeyBindType.Toggle)));             
        ...
        {
            config.AddItem(new MenuItem("spellDelay" + Id, "法術延遲").SetValue(new Slider(500, 0, 3000)));
            config.AddItem(
                new MenuItem("CastR" + Id, "投擲 R").SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Press)));
        ...
        public override bool DrawingMenu(Menu config)
        {
            ...
            config.AddItem(new MenuItem("DrawQC" + Id, "最小 Q 距離").SetValue(new Circle(true, Color.White)));
            config.AddItem(new MenuItem("DrawRS" + Id, "R: 搜索區域").SetValue(new Circle(true, Color.White)));
