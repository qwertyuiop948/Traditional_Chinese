    ...
    internal class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            var summonersHeal = summoners.AddSubMenu(new Menu("治愈", "Heal"));
            {
                ...
                summonersHeal.AddItem(new MenuItem("SUMHEALSLIDER", "血量低於.").SetValue(new Slider(20, 99, 1)));
            }
            ...
            var summonersBarrier = summoners.AddSubMenu(new Menu("屏障", "Barrier"));
            {
                ...
                summonersBarrier.AddItem(
                    new MenuItem("SUMBARRIERSLIDER", "血量低於.").SetValue(new Slider(20, 99, 1)));
            }

            var summonersIgnite = summoners.AddSubMenu(new Menu("點燃", "Ignite"));
            {
                summonersIgnite.AddItem(new MenuItem("SUMIGNITEENABLE", "啟用").SetValue(true));
            }
            /* Menu Items */            
            var items = Config.AddSubMenu(new Menu("項目", "Items"));
            items.AddItem(new MenuItem("BOTRK", "破敗").SetValue(true));
            items.AddItem(new MenuItem("GHOSTBLADE", "鬼刀").SetValue(true));
            items.AddItem(new MenuItem("SWORD", "神聖之劍").SetValue(true));
            items.AddItem(new MenuItem("MURAMANA", "魔宗利刃").SetValue(true));
            QuickSilverMenu = new Menu("QSS", "水銀飾帶");
            items.AddSubMenu(QuickSilverMenu);
            QuickSilverMenu.AddItem(new MenuItem("AnyStun", "眩暈").SetValue(true));
			QuickSilverMenu.AddItem(new MenuItem("AnySlow", "減速").SetValue(true));
            QuickSilverMenu.AddItem(new MenuItem("AnySnare", "夾子").SetValue(true));
            QuickSilverMenu.AddItem(new MenuItem("AnyTaunt", "嘲諷").SetValue(true));
            ...
            items.AddItem(
                new MenuItem("UseItemsMode", "使用物品").SetValue(
                    new StringList(new[] {"no", "混合", "連招", "全部"}, 2)));
            ...
            if (BaseType != CClass.GetType())
            {
                ...
                {
                    harass.AddItem(new MenuItem("HarassMana", "藍量控制%").SetValue(new Slider(50, 100, 0)));
                ...
                {
                    laneclear.AddItem(
                        new MenuItem("LaneClearMana", "藍量控制").SetValue(new Slider(50, 100, 0)));
                ...
                var drawing = new Menu("顯示", "Drawings");
				...
				Config.AddSubMenu(new Menu("超神漢化", "by weilai"));
				Config.SubMenu("by weilai").AddItem(new MenuItem("qunhao", "漢化群：386289593"));				
            }
