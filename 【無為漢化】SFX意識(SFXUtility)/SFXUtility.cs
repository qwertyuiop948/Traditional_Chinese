    ...
    internal class SFXUtility
    {
        ...
        public SFXUtility()
        {
            try
            {
                ...
                var miscMenu = new Menu("無為漢化─雜項", "Misc");

                var infoMenu = new Menu("L#漢化信息", "Info");
                infoMenu.AddItem(new MenuItem("InfoVersion", string.Format("版本: {0}", Version)));
                infoMenu.AddItem(new MenuItem("InfoIRC", "作者∶ Appril"));
				infoMenu.AddItem(new MenuItem("qun", "L#漢化群∶386289593"));
                ...
                miscMenu.AddItem(new MenuItem("MiscCircleThickness", "線圈厚度").SetValue(new Slider(3, 10, 1)));
